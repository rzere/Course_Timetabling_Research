...
LINEAR_COMBO_CONFLICT = 25
LINEAR_COMBO_PROXIMITY = 1
LINEAR_COMBO_GOOD_TO_BAD_SWITCH = 25
LINEAR_COMBO_GOOD_TO_BAD_ROOMS = 0

NUM_VERTICES_TO_EXPAND = 1
NUM_COLORS_PER_VERTEX = 1
MAX_QUEUE_LENGTH = 25

PRIORITY_TOTAL_EDGE_WEIGHT = 1
PRIORITY_NUM_EDGES_WEIGHT = 1
PRIORITY_TOTAL_BAD_VALUE_WEIGHT = 10
PRIORITY_TOTAL_PENALTY_WEIGHT = 1
...
def convert_time(time):
    ...
    if 'pm' in time_fields[1] and hours < 12:
        hours += 12
    if 'am' in time_fields[1] and hours > 11:
        hours -= 12
...
def calculate_total_penalty(solution, edges, overlapping_timeslots, timeslot_gaps):
...
def one_pass_solver(vertices, edges, overlapping_timeslots, timeslot_gaps, solution = {}):
...
def expand(solution, n, vertices, edges, overlapping_timeslots, timeslot_gaps):
    ...
    for vertex in keys:
        ...
        # Immediately schedule vertices with only one timeslot option
    ...
    for vertex in vertices:
        ...
        for neighbor in neighbors:
            ...
            if weight == 'H' or weight == 'I':
                total_edge_weight += HEAVY_CONFLICT_PENALTY
            if weight == 'M':
                total_edge_weight+= MEDIUM_CONFLICT_PENALTY
            if weight == 'L':
                total_edge_weight += LIGHT_CONFLICT_PENALTY
                
            number_of_edges += 1
    
    return total_bvoc, total_edge_weight, number_of_edges
...
def priority_queue_solver(vertices, edges, overlapping_timeslots, timeslot_gaps):
    ...
    while len(queue) > 0:
        ...
        if len(list_of_vertices) == 0:            
            penalty = calculate_total_penalty(current_solution, edges, overlapping_timeslots, timeslot_gaps)
        ...
        for vertex in list_of_vertices:
            ...
            for priority, color, room in selections:
                ...
                #total_bvoc, total_edge_weight, num_edges = priority_function(new_solution, 
                #                                            new_vertices, edges, 
                #                                            overlapping_timeslots, 
                #                                            timeslot_gaps)
                
                one_pass_priority = one_pass_solver(new_vertices, edges, overlapping_timeslots, timeslot_gaps, new_solution)
                                
                penalty = calculate_total_penalty(new_solution, edges, overlapping_timeslots, timeslot_gaps)
                ...
                solution_score = penalty + one_pass_priority
...
def read_courses(f):
    ...
    while line != '\n':
        ...
        # The first and last entries of the room
        rooms = info[4].split(' ')
...
def schedule_ctb_file(input_file_name):
    ...
    penalty = calculate_total_penalty(solution, edges, overlapping_timeslots, timeslot_gaps)
...
def schedule_ctb_file_with_parameters(individual, file_path):
    ...
    global LINEAR_COMBO_EDGE_CONFLICT_AVOIDANCE
    global LINEAR_COMBO_ROOM_CONFLICT_AVOIDANCE
    global LINEAR_COMBO_CONFLICT
    global LINEAR_COMBO_PROXIMITY

    # Set global parameters
    LINEAR_COMBO_EDGE_CONFLICT_AVOIDANCE = individual[0]
    LINEAR_COMBO_ROOM_CONFLICT_AVOIDANCE = individual[1]
    LINEAR_COMBO_CONFLICT = individual[2]
    LINEAR_COMBO_PROXIMITY = individual[3]
...
if __name__ == "__main__":
    ...
    #base_dir = '/Users/dmyers/Dropbox/DanJayRuz/Test_Problems/'
    ...
    s = 0
    for i in range(10):
        full_path = base_dir + file_name + str(i + 5) + '.ctb'
